...
# Arg 5 = Original blasted sequence names file
...
my ($csv,$seq,$taxfile,$cutoff,$outputdir,$namesfile)=@ARGV;
my @csv =();
...
unless (-e $namesfile) {
	print $log "Names file doesn't exist\nOperation aborted\n\n";
	print "Names file doesn't exist\nOperation aborted\n\n";
	die
}
...
unless (open($handle,$seq)) {print "No such seq file\n";die}
...
while (my $record=<$handle>) {
        ...
        if (exists $lookup{$name}) {
			...
			# Look for record in names file
			$count = `grep -P "$name\t" $namesfile`;
			$count =~ s/^.*\t//;
			@records = split /,/, $count;
			foreach my $element (@records) {
				chomp $element;
				print $outhandle ">$element\n";
				print $outhandle "$hitsequence\n";
				print $aaouthandle ">$element\n$transsequence\n";
				print $log "\nWrote $element to $output and $aaoutput\n";
			}
